  ...
  const MintBlogger = {
    ...
    comments: {
      ...
      init: function() {
        ...
        if (this.container) {
          this.replyButtons = this.container.querySelectorAll("[data-parent-id]");
          if (this.form) {
            this.handleComments();
          }
        }
      ...
      extractFormSrc: function(formHtml) {
        if (!formHtml) return { originalSrc: '', form: '' };
        const match = formHtml.match(/<iframe[^>]*\s+src="([^"]*)"/i);
        const originalSrc = match ? match[1] : '';
        return { originalSrc, form: formHtml };
      },
      ...
      handleComments: function() {
        ...
        const updateReplyForm = (parentId) => {
          const newSrc = `${originalSrc}&parentID=${parentId}`;
          replyForm.innerHTML = form.replace(originalSrc, newSrc);
        };
        ...
        this.formRestore?.addEventListener('click', () => {
          if (activeBtn) {
            activeBtn.classList.remove("is-active");
            activeBtn = null;
            document.getElementById("reply-form")?.remove();
