...
(function(window, document) {
  ...
  const MintBlogger = {
    ...
    comments: {
      ...
      REPLYING_CLASS: 'is-replying',
      HAS_REPLY_FORM: 'has-reply-form',
      MARGIN: '200px',

      init: function() {
        this.container = document.getElementById('post-comment');
        this.loadMoreButton = document.getElementById('load-more-comments');
        this.commentForm = document.getElementById('comment-form');
        this.formScript = document.getElementById('form-script');
        this.formRestore = document.getElementById('form-restore');
        this.replyButtons = document.querySelectorAll('[data-parent-id]');
      ...
      extractFormSrc: function(formHtml, newSrc) {
        if (!formHtml) return;
        const originalSrc = formHtml.match(/<iframe[^>]*\s+src="([^"]*)"/i)[1];
        return {
          originalSrc,
          form: newSrc ? formHtml.replace(originalSrc, newSrc) : formHtml
        };
      },

      initializeCommentSystem: function() {
        this.originalForm = this.commentForm.innerHTML;
        const {originalSrc, form} = this.extractFormSrc(this.originalForm);
        ...
        this.replyButtons?.forEach(btn => {
          btn.onclick = () => {
            ...
            const existingReplyForm = document.getElementById('reply-form');

            if (this.activeBtn === btn) return;

            if (existingReplyForm) {
              existingReplyForm.parentElement.classList.remove(this.HAS_REPLY_FORM);
              existingReplyForm.remove();
            } else {
              this.commentForm.innerHTML = '';
              this.formRestore.classList.add(this.REPLYING_CLASS);
            }

            if (this.activeBtn) {
              this.activeBtn.classList.remove(this.ACTIVE_CLASS);
            }

            const newSrc = `${originalSrc}&parentID=${parentId}`;
            const {form: newForm} = this.extractFormSrc(this.originalForm, newSrc);

            btn.classList.add(this.ACTIVE_CLASS);
            this.activeBtn = btn;

            replyForm.innerHTML = newForm;
            repliesContainer.classList.add(this.HAS_REPLY_FORM);
            repliesContainer.insertAdjacentElement('afterbegin', replyForm);
          };
        });

        this.formRestore?.onclick = () => {
          if (this.activeBtn) {
            const replyForm = document.getElementById('reply-form');
            this.formRestore.classList.remove(this.REPLYING_CLASS);
            this.commentForm.innerHTML = form;
            this.activeBtn.classList.remove(this.ACTIVE_CLASS);
            this.activeBtn = null;
            replyForm.parentElement.classList.remove(this.HAS_REPLY_FORM);
            replyForm.remove();
          }
        };
      },

      loadDisqus: function(element) {
        const {shortname, postUrl, postId} = element.dataset;
      ...
      initDisqus: function() {
        ...
        const disqusCount = document.querySelector('.disqus-comment-count');

        if (disqusCount) {
          const {shortname} = disqusCount.dataset;
        ...
        if (disqusThread) {
          if (disqusButton) {
            ...
            this.observeElement(disqusThread, () => {
              this.loadDisqus(disqusThread);
      ...
      observeElement: function(element, callback, options) {
      ...
      updateLoadMoreButton: function(isLoading) {
        this.loadMoreButton.textContent = isLoading ? 
          MintBlogger.util.translate('loading') : 
          MintBlogger.util.translate('loadMore');
