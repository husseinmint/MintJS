...
(function(window, document) {
  ...
  const MintBlogger = {
    ...
    comments: {
      loadCommentsForPost: function(postId) {
        ...
        commentsContent.innerHTML = '<p>' + MintBlogger.util.translate('loading') + '</p>';

        MintBlogger.util.fetchJSON(`/feeds/comments/default?alt=json&postId=${postId}`)
          .then(data => {
            const comments = data.feed.entry || [];
            commentsContent.innerHTML = ''; // Clear loading message
            if (comments.length === 0) {
              commentsContent.innerHTML = '<p>' + MintBlogger.util.translate('noMoreComments') + '</p>';
              return;
            }
          ...
          .catch(error => {
            ...
            commentsContent.innerHTML = '<p>' + MintBlogger.util.translate('errorOccurred') + '</p>';
          });
      },
      ...
      createCommentElement: function(comment) {
        ...
        return MintBlogger.util.createElement('div', { className: 'comment' }, [
          MintBlogger.util.createElement('div', { className: 'comment-header' }, [
            MintBlogger.util.createElement('img', { className: 'comment-avatar', src: authorImage, alt: author }),
            MintBlogger.util.createElement('div', { className: 'comment-meta' }, [
              MintBlogger.util.createElement('span', { className: 'comment-author' }, [author]),
      ...
      handleCommentSubmit: function(event) {
        ...
        const newComment = {
          author: [{ name: { $t: 'Current User' }, gd$image: { src: 'https://www.blogger.com/img/blogger_logo_round_35.png' } }],
          content: { $t: commentText },
