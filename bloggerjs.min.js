  ...
  const MintBlogger = {
    ...
    comments: {
      ...
      init: function(options = {}) {
        this.container = document.getElementById('post-comments');
        this.loadMoreButton = document.getElementById('load-more-comments');
        
        if (this.container) {
          this.template = this.container.querySelector('[data-bjs="template"]');
          if (this.template) {
            this.template.removeAttribute('data-bjs');
            this.template = this.template.innerHTML;
          }

          this.options = {
            maxResults: MintBlogger.config.commentsPerPage,
            ...options,
            ...MintBlogger.util.getDataAttributes(this.container)
          };
      ...
      fetchComments: async function() {
        const apiUrl = `${document.location.origin}/feeds/comments/default?alt=json&orderby=published&start-index=${(this.currentPage - 1) * this.options.maxResults + 1}&max-results=${this.options.maxResults}`;
        return await MintBlogger.util.fetchJSON(apiUrl);
      },
      ...
      createCommentElement: function(comment) {
        const commentData = this.parseCommentData(comment);
        if (this.template) {
          return MintBlogger.util.createElement('div', { 
            className: 'mintblogger-comment',
            innerHTML: this.replaceTemplate(this.template, commentData)
          });
        } else {
          return this.createDefaultCommentElement(commentData);
        }
      },

      parseCommentData: function(comment) {
        return {
          author: comment.author[0].name.$t,
          author_url: comment.author[0].uri ? comment.author[0].uri.$t : '#',
          author_image: this.getAuthorImage(comment),
          content: comment.content.$t,
          date: MintBlogger.util.formatDate(comment.published.$t),
          post_id: comment.id.$t.split('post-')[1],
          url: comment.link.find(link => link.rel === 'alternate').href
        };
      },

      getAuthorImage: function(comment) {
        const authorId = comment.author[0].gd$image.src;
        let authorImage = 'https://lh3.googleusercontent.com/a/default-user';
        if (!authorId.includes('g/blank.gif') && !authorId.includes('g/b16-rounded.gif')) {
          authorImage = authorId.replace(/\/s\d+-c\//, '/s80-c/');
        }
      ...
      createDefaultCommentElement: function(commentData) {
        return MintBlogger.util.createElement('div', { className: 'mintblogger-comment' }, [
          MintBlogger.util.createElement('div', { className: 'comment-author' }, [
            MintBlogger.util.createElement('img', { src: commentData.author_image, alt: commentData.author, className: 'comment-avatar' }),
            MintBlogger.util.createElement('a', { href: commentData.author_url, className: 'comment-author-name' }, [commentData.author])
          ]),
          MintBlogger.util.createElement('div', { className: 'comment-content', innerHTML: commentData.content }),
          MintBlogger.util.createElement('div', { className: 'comment-meta' }, [
            MintBlogger.util.createElement('span', { className: 'comment-date' }, [commentData.date]),
            MintBlogger.util.createElement('a', { 
              href: 'javascript:void(0);', 
              className: 'comment-reply',
              onclick: `MintBlogger.comments.handleReply('${commentData.post_id}')`
      ...
      handleReply: function(commentId) {
        const replyForm = document.getElementById('comment-editor');
        if (replyForm) {
          replyForm.src = replyForm.src.split('#')[0] + '#c' + commentId;
